...
class NetworkxInspector(GraphInspector):
    ...
    def inspect(self) -> GraphProfile:
        """Inspect the graph and return a frozen GraphProfile."""
        node_type_profiles = self._inspect_nodes()
        rel_type_profiles = self._inspect_relationships(node_type_profiles)
        return GraphProfile(
            source="networkx",
    ...
    def _inspect_nodes(self) -> dict[str, NodeTypeProfile]:
    ...
    def _inspect_relationships(
        ...
        for src, tgt, attrs in self._graph.edges(data=True):
            ...
            src_attrs = self._graph.nodes.get(src, {})
            tgt_attrs = self._graph.nodes.get(tgt, {})
