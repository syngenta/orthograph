"""Typed query generics and Executor seam.
...
def _enforce_query_contract(
    ...
    """Validate that a concrete query subclass declares its required class vars.

    Checks two things per required attribute:
      1. Presence — the attribute is assigned somewhere in the MRO.
      2. Type — ``model_attrs`` must be ``BaseModel`` subclasses; ``backend`` (if
         present in ``other_attrs``) must be a ``Backend`` value.

    Raises ``TypeError`` listing every problem found.
    """
    problems: list[str] = []
...
class ReadQuery(ABC, Generic[P, D]):
    ...
    def build(self, params: P) -> Any:
        """Pure, I/O-free construction of the backend query object.

        Must NOT open a connection, read from a file, or produce side effects.
        """
...
class WriteQuery(ABC, Generic[P, R]):
    ...
    are expressed via the ``R`` TypeVar and ``interpret_result()``.
    """
...
class Executor(ABC):
    ...
    ``read()`` and ``write()`` are DISTINCT methods — they carry different
    transactional intent and must not be unified behind a kind flag.

    Implementations must not store a live connection as instance state. A
    factory callable is passed at construction so the executor opens and closes
    sessions per call.

    Example (neo4j driver)::

        driver = GraphDatabase.driver(URI)
        executor = CypherExecutor(driver.session)
    ...
    def read(self, query: ReadQuery[P, D], raw_params: Any) -> list[D]:
        """Validate params → build() (pure) → open session → execute →
        materialise each record. Commits nothing.
        """
