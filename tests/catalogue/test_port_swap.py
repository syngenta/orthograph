"""T6 — integration proof: the same ReadPort yields identical domain objects
regardless of the raw record shape behind it.

This is the swappable-read claim from STEP 1/2 demonstrated end to end. A true
cross-backend swap (Cypher vs SQL) needs E14's ``SqlExecutor``; until then we
prove the property with two Cypher executors fed differently-shaped records, and
with two distinct queries that share one ``Output`` model bound behind the same
``ReadPort[P, D]`` annotation.

When E14 lands, extend this with a real Cypher-vs-SQL port swap (cross-ref E14).
"""
...
from orthograph.extensions.cypher import CypherExecutor, CypherReadQuery
