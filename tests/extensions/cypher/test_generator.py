...
def test_merge_node_rejects_injected_property_key(model: GraphDataModel):
...
def test_create_node_rejects_injected_property_key(model: GraphDataModel):
    """An injected property key raises before any Cypher string is returned.
...
def test_create_node_rejects_injected_label(model: GraphDataModel):
...
def test_create_relationship_rejects_injected_property_key(model: GraphDataModel):
...
class _InjectedRel(RelationshipModel):
    ...
    __source_type__ = _GoodSource
    __target_type__ = _GoodTarget
...
def test_merge_node_without_uid_field_falls_back_through_merge_node():
    ...
    model = GraphDataModel(name="m", node_types=[_NoUidNode], relationship_types=[])
    gen = CypherGenerator(model)
...
def test_merge_node_rejects_unknown_label(model: GraphDataModel):
...
def test_create_relationship_rejects_unknown_label(model: GraphDataModel):
...
def test_merge_node_rejects_undeclared_property(model: GraphDataModel):
...
def test_merge_node_undeclared_property_error_names_label(model: GraphDataModel):
...
def test_create_node_rejects_undeclared_property(model: GraphDataModel):
...
def test_merge_node_declared_properties_unchanged(model: GraphDataModel):
...
def test_create_relationship_rejects_undeclared_property(model: GraphDataModel):
...
def test_create_relationship_declared_property_unchanged(model: GraphDataModel):
...
#
# The generator can emit E16 CypherReadQuery / CypherWriteQuery instances that
# register in a QueryCatalogue, carry Params/Output models, bake the model-fixed
# label as a validated literal, and pass E16's definition-time $param ↔ Params
# alignment check.
...
def test_match_by_uid_query_returns_typed_read(model: GraphDataModel):
...
def test_match_by_uid_query_references_only_model_identifiers(model: GraphDataModel):
...
def test_match_by_uid_query_passes_definition_time_validation(model: GraphDataModel):
...
def test_merge_query_returns_typed_write(model: GraphDataModel):
...
def test_create_query_returns_typed_write(model: GraphDataModel):
...
def test_delete_by_uid_query_produces_detach_delete(model: GraphDataModel):
...
def test_match_by_uid_query_materialize_maps_node_record(model: GraphDataModel):
...
def test_generated_queries_register_in_catalogue(model: GraphDataModel):
    """All four generated queries register and describe() with correct kinds."""
    gen = CypherGenerator(model)
    catalogue = QueryCatalogue()
    catalogue.register_read(gen.match_by_uid_query(Person))
    catalogue.register_write(gen.merge_query(Person))
    catalogue.register_write(gen.create_query(Person))
    catalogue.register_write(gen.delete_by_uid_query(Person))

    descriptions = catalogue.describe()
...
def test_typed_queries_require_uid_field(model: GraphDataModel):
...
# --- interpret_result: affected-node counts from the driver result ---------
...
def test_create_query_interpret_result_reads_nodes_created(model: GraphDataModel):
...
def test_merge_query_interpret_result_reads_nodes_created(model: GraphDataModel):
...
def test_delete_by_uid_query_interpret_result_reads_nodes_deleted(
    model: GraphDataModel,
):
...
def test_interpret_result_accepts_mapping_shaped_result(model: GraphDataModel):
...
# GraphDataModel as its last step. These tests verify that this guarantee
...
def test_raw_merge_node_validates_against_model(model: GraphDataModel):
...
def test_model_validation_error_carries_issues(model: GraphDataModel):
...
def test_model_validation_error_in_match_node_on_unregistered_type():
    ...
    empty_model = GraphDataModel(name="e", node_types=[], relationship_types=[])
...
def test_typed_match_by_uid_query_validates_against_model(model: GraphDataModel):
...
def test_model_validation_error_message_lists_issues(model: GraphDataModel):
