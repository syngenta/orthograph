"""Tests for orthograph.core.relationship_model -- RelationshipModel base class."""
...
def test_relationship_model_simple():
    ...
    assert ActedIn.__source_type__ is Person
...
def test_relationship_model_requires_source_type():
    with pytest.raises(TypeError, match="__source_type__"):
...
def test_relationship_model_requires_target_type():
    with pytest.raises(TypeError, match="__target_type__"):
...
def test_relationship_model_self_referencing():
    ...
    assert Manages.__source_type__ is Person
    assert Manages.__target_type__ is Person
