...
| E58 | Comparison Dispatch & Rule Symmetry — Scope & Decide | Low | planned (decision-first, no fix until ADR; origin E56 W2 deferred Note; two design items the behaviour-preserving E56 could not touch — (A) the satisfaction-path `left`/`right` argument inversion in `compare_profile_to_definition`, documented but enforced nowhere; (B) the `rules.py`↔`diff_rules.py` type-mismatch/operand-kind symmetry — consolidate only if net-deleting, mirroring E56.6; E58.0 produces an ADR recording change-vs-no-change + do-and-undo risk; the one in-scope crumb — the "five-pass"→"four-pass" comment — was already fixed in E56.5) |
| E59 | Query Validation Public API — Split and Expose the Three Validation Paths | Low | planned (ADR-first; origin E56 distillation; `validate_cypher_query` + `validate_typed_cypher_query` are named and ready internally — E59.0 decides whether to split/deprecate `validate_query`, then E59.1–E59.3 expose the new verbs; depends on E56 + E55 done) |
| E60 | Query Shape Alignment — One Vocabulary Across Typed / Cypher / ORM Paths | Medium | planned (ADR-first; origin E56 W4 re-analysis 2026-06-28; the typed path (`Params`/`Identifiers`/`name`) and `CypherQuery` (`params_schema`/`identifiers_schema`/`query_id`) diverge by *attribute name only* — `cypher_template` already shared — and are reconciled at five sites via a `ReadQuery|WriteQuery|CypherQuery` union + `getattr`/`isinstance`; **ratified direction: typed adopts the Cypher names** (their YAML wire format), aligning the vocabulary across parallel paths WITHOUT unifying the types; deletes `CypherExecutor._query_shape` + the catalogue/validation reconciliation; public-attribute + serialization-adjacent design change; records the latent bug in the original E56.15 adapter-routing instruction; depends on E56 done + sequenced vs E59) |
...
- [E59 — Query Validation Public API — Split and Expose the Three Validation Paths](active_epics/E59_query_validation_public_api.md) *(ADR-first; origin E56 distillation; `validate_cypher_query` + `validate_typed_cypher_query` named and ready internally; decides split/deprecation of `validate_query`)*
- [E60 — Query Shape Alignment — One Vocabulary Across Typed / Cypher / ORM Paths](active_epics/E60_query_shape_alignment.md) *(ADR-first; origin E56 W4 re-analysis; typed path adopts the Cypher names `params_schema`/`identifiers_schema`/`query_id` to align the vocabulary across parallel paths without unifying the types; deletes the 5 reconciliation sites; records the latent bug in the original E56.15)*
