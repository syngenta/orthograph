...
| E48 | Configuration — Thread Tunable Knobs Through the Public API | Medium | planned (**blocked by E55, the new-API epic**; E48.0 ADR-036 may begin alongside the API decision; exposes E46's `severity_threshold` + inspector `value_counts_top_n`/`strategy`; no global settings/file/env layer this phase)                                                                                                                              |
...
AFTER the new-API epic (E55):
  E48  Configuration (exposes E46's PropertyTypeMismatchRule.severity_threshold + inspector
       ...
       alongside the API decision; E55 reserves the config-ready entry-point
