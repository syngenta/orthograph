...
| Where do vendor backends and the vendor-free inspection currency live? | `src/orthograph/backends/<vendor>/` (adapters + queries) and `src/orthograph/graph_profile/` (GraphProfile, inspector ABC); the declared side is `src/orthograph/graph_definition/`, the cross-layer comparison is `src/orthograph/comparison/` (`compare`), and the shared result currency is `src/orthograph/diagnostics/`. The Cypher language tool is top-level `src/orthograph/cypher/`. *(Topology per ADR-017.)* |
| How does comparison work? How do I add a new validation rule? | `src/orthograph/comparison/engine.py` (engine) + `src/orthograph/comparison/rules.py` (Rule protocol, standard_rules, Case-A/B extension recipes) + [decisions/015-declared-observed-mirror.md](decisions/015-declared-observed-mirror.md) |
